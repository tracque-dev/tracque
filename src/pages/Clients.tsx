  ...
  return (
    <div className="p-7 space-y-6 max-w-[1400px]">
      <div className="flex items-end justify-between">
        ...
        <button onClick={() => setAdding(a => !a)} className="flex items-center gap-2 bg-foreground text-background px-4 py-2.5 rounded-xl text-sm font-medium hover:opacity-90 transition-opacity">
          <Plus className="w-4 h-4" /> New client
        </button>
      </div>

      {adding && (
        <form onSubmit={handleAdd} className="bg-card border border-border rounded-2xl p-5 shadow-card space-y-3">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {clients.map(c => (
            <div key={c.id} className="group bg-card border border-border rounded-2xl p-5 shadow-card hover:shadow-card-md transition-shadow">
              ...
              <button onClick={() => openClient(c.id)} className="mt-4 w-full flex items-center justify-center gap-1.5 text-sm font-medium text-foreground border border-border rounded-xl py-2 hover:bg-muted transition-colors">
