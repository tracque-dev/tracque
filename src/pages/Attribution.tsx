...
function StatCard({ icon: Icon, label, value, tint }: { icon: any; label: string; value: string; tint: string }) {
  return (
    <div className="bg-card rounded-xl border border-border p-5">
      <div className={`w-9 h-9 rounded-lg ${tint} flex items-center justify-center mb-3`}><Icon className="w-4 h-4" /></div>
      <p className="text-2xl font-display font-semibold nums tracking-tight">{value}</p>
    ...
    <div className="p-7 space-y-6 max-w-[1400px]">
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="text-2xl font-display font-semibold tracking-tight mt-1">Attribution</h1>
      ...
      {!site ? (
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-5">
          <p className="text-sm font-display font-semibold text-primary mb-1">Set up conversion tracking</p>
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div>
              ...
              <input value={domain} onChange={e => setDomain(e.target.value)} placeholder="acme.com" className="mt-1 w-44 px-3 py-2 text-sm border border-border rounded-lg bg-background focus:ring-primary" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground">GA4 ID (optional)</label>
              <input value={ga4} onChange={e => setGa4(e.target.value)} placeholder="G-XXXXXXX" className="mt-1 w-40 px-3 py-2 text-sm border border-border rounded-lg bg-background focus:ring-primary" />
            </div>
            <button onClick={() => createSite.mutate({ domain: domain || undefined, ga4_id: ga4 || undefined })} disabled={createSite.isPending}
              className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-40 transition-all">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={DollarSign} label="Total revenue" value={money(totalRevenue)} tint="bg-secondary text-foreground" />
        <StatCard icon={MousePointerClick} label="AI-driven revenue" value={money(aiRevenue)} tint="bg-primary/10 text-primary" />
        <StatCard icon={TrendingUp} label="Conversions" value={totalConversions.toLocaleString()} tint="bg-secondary text-foreground" />
        <StatCard icon={Users} label="Sessions" value={totalSessions.toLocaleString()} tint="bg-secondary text-foreground" />
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {sorted.map(s => {
                ...
                return (
                  <tr key={s.source} className="border-b border-border last:border-0 hover:bg-white/[0.02] transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${color(s.source)}`} />
                        <span className="text-sm font-medium">{label(s.source)}</span>
                        {isAi && <span className="text-[10px] font-mono px-1.5 py-0.5 bg-primary/10 text-primary rounded-md">AI</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 bg-white/[0.06] rounded-full overflow-hidden"><div className={`h-full ${color(s.source)} rounded-full`} style={{ width: `${pct}%` }} /></div>
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'AI Search', match: (s: SourceAttribution) => s.is_ai || AI_SOURCES.has(s.source), color: 'bg-primary' },
            { label: 'Paid Ads', match: (s: SourceAttribution) => s.source.startsWith('paid_'), color: 'bg-primary/40' },
            { label: 'Organic / Direct', match: (s: SourceAttribution) => ['google', 'bing', 'referral', 'direct'].includes(s.source), color: 'bg-white/25' },
          ].map(g => {
            ...
            return (
              <div key={g.label} className="bg-card rounded-xl border border-border p-5">
                ...
                <p className="text-2xl font-display font-semibold nums tracking-tight">{money(rev)}</p>
